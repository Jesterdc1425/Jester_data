...
# Function to handle Word document upload and text extraction
def handle_word_document_upload(uploaded_file):
    """
    Extract text from uploaded Word document (.docx)
    ...
    try:
        ...
        extracted_text = re.sub(r'\n\s*\n', '\n\n', extracted_text)  # Replace multiple newlines
        extracted_text = re.sub(r'[ \t]+', ' ', extracted_text)  # Replace multiple spaces/tabs
...
with st.sidebar:
    ...
    # Updated branding - Zaidyn Security Team
    st.markdown("**Developed by Zaidyn Security Team**")
...
""")
    ...
    with col1:
        # Add Word document uploader
        st.subheader("Upload Word Document")
        uploaded_word_file = st.file_uploader(
            "Upload a Word document (.docx) with application details",
            type=["docx"],
            help="Upload a Word document containing your application description, architecture, or requirements. The text will be automatically extracted and loaded into the application description field."
        )
        
        # Process uploaded Word document
        if uploaded_word_file is not None:
            with st.spinner("Extracting text from Word document..."):
                extracted_text = handle_word_document_upload(uploaded_word_file)
                if extracted_text:
                    # Auto-fill the application description with extracted text
                    st.session_state['app_input'] = extracted_text
                    st.success("✅ Word document content has been loaded into the application description field below.")
                    
                    # Show a preview of the extracted text
                    with st.expander("View extracted text preview"):
                        preview_text = extracted_text[:500] + "..." if len(extracted_text) > 500 else extracted_text
                        st.text_area("Extracted Text Preview", preview_text, height=150, key="preview")
